...
public:
    ...
    void enqueue()
    {
        for (auto* _childNode : _nextNodes)
            _childNode->parentIsEnqueued();
    }
    virtual void parentIsEnqueued() = 0;
...
protected:
    // enqueue is being done by main thread so there wont be race issue here
...
private:
    virtual void parentIsEnqueued() final;
...
public:
    ...
    void initialize()
    {
        for (auto& nodePtr : _nodes)
            if (!nodePtr->getPendingCount())
                _initialNodes.push_back(nodePtr.get());
    }
    /// @brief run the graph execution from input nodes
    void execute()
    {
        if (_initialNodes.empty())
            initialize();
        for (auto& initialNode : _initialNodes) {
            _pool.push([initialNode]([[maybe_unused]] int id) -> void {
                initialNode->execute();
            });
            initialNode->enqueue();
        }
    ...
    void enqueSingleNode(NodeType* node)
    {
        _pool.push(
            [node]([[maybe_unused]] int id) -> void { node->execute(); });
        node->enqueue();
    }
...
private:
    ...
    std::vector<BaseNode*> _initialNodes;
};

template <typename TaskCallback, typename... Args>
void Node<TaskCallback, Args...>::parentIsEnqueued()
{
    if (++parentEnqueued == _parentCount)
        _executor->enqueSingleNode(this);
}
